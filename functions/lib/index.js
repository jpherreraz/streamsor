...
exports.cleanupStaleStreams = (0, scheduler_1.onSchedule)('every 15 minutes', async (event) => {
    const db = (0, database_1.getDatabase)();
        ...
        for (const [streamId, stream] of Object.entries(snapshot.val() || {})) {
            ...
            if (streamData.status === 'live') {
                try {
                    ...
                    const response = await checkCloudflareWithRetry(`https://api.cloudflare.com/client/v4/accounts/${cloudflareAccountId.value()}/stream/live_inputs/${liveInputId}`, cloudflareApiToken.value());
                    if (!response.ok) {
...
exports.getActiveStreams = (0, https_1.onCall)({
    ...
    try {
        ...
        const checkStreamStatus = async (streamer) => {
            try {
                console.log('Checking Cloudflare status for streamer:', streamer.uid, streamer.liveInputId);
                const response = await checkCloudflareWithRetry(`https://api.cloudflare.com/client/v4/accounts/${cloudflareAccountId.value()}/stream/live_inputs/${streamer.liveInputId}`, cloudflareApiToken.value());
